...
- The **Scheduled Task Inventory** tab automatically runs a fast non-verbose query the first time it is opened. It hides Microsoft Windows, disabled, and empty task entries by default, supports filtering and sortable columns, and loads verbose details only when a row is selected. The empty-task filter only removes rows with no meaningful status, schedule, action, or descriptive metadata. Right-click a task to view a plain-language detail window, enable or disable it, or permanently delete its task definition. Microsoft tasks under `\Microsoft\Windows` are protected from changes, and deletion always requires confirmation. A slower verbose-initial-query option is available for later manual refreshes. Local queries omit `/s`; remote targets use the current Windows identity with `/s HOSTNAME`.
